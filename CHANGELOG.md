...
- Added `mierus://` import/export with repeated TCP/UDP port bindings and port ranges.
- Added native Mihomo YAML generation with SG routing, DNS and TUN integration.
- Added Mieru profile display, filtering, duplicate comparison and endpoint latency handling.
- Added unit tests for Mieru format parsing and Mihomo configuration generation.
- Finalized the Luxury Jade Depth light theme with vertical gradients, soft card shadows and restrained highlight edges.
- Preserved the original Graphite and Northern palettes without the rejected experimental dark-depth treatment.
- Preserved SG Client 095 connection modes, routing, traffic accounting, tray controls, subscriptions, GeoFiles and diagnostics.
- Confirmed the final Windows build and approved the resulting interface.
